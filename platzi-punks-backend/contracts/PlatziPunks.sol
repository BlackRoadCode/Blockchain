...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA{
    ...
    constructor( uint256 _maxSupply ) ERC721("Plazti Punks Token", "PTP"){ 
    ...
    function tokenURI( uint256 tokenId ) public view override returns( string memory){
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name":"PlatziPunks #',
                tokenId.toString(),
                '", "description": "Descripcion del platzi punk con url encodeada", "image":"',
                image,
                '" }'
            )
        );
    ...
    // Override required for ennumerable support
